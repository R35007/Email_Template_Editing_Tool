...
$( document ).ready( function ()
{
    ...
    {
        $DesignIframe = $( '#DesignView' ).contents();
        DesignViewEvents( $DesignIframe )
    } )

    function DesignViewEvents ( $DesignIframe )
    {
        //  Create Editor
        ...
        editor.session.on( 'change', function ( delta )
        {
            ...
            var body = htmlVal.replace( /^.*?<body bgcolor="#e6e6e6" style="margin: 0px; padding: 0px;">(.*?)<\/body>.*?$/s, "$1" );

            $DesignIframe.find( 'html' ).html( body );
            $DesignIframe = $( '#DesignView' ).contents();
            DesignViewEvents( $DesignIframe );
    ...
    $( '.navlist' ).on( 'click', function ()
    {
        ...
        } else if ( tabs == "SourceView" )
        {
            ...
            $( this ).addClass( 'activeTab' );
            $( '#Design' ).css( 'height', 'calc(100% - 72px)' );
        } else if ( tabs == "SplitView" )
        {
            myDesign = $DesignIframe.find( 'html' ).html();
    ...
    {
        var $DesignView = $DesignIframe.find( 'html' ).clone();
